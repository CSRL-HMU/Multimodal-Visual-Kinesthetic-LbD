...
# computing JQlog from:
# A correct formulation for the Orientation Dynamic
# Movement Primitives for robot control in the Cartesian space
def Jlog(Q):
    ...
    if np.abs(theta)>0.000000001:
        Jl[0,0:3] = - stheta * k.T
        Jl[1:4,0:3] = (stheta/theta) * (I3 - k @ k.T) + ctheta * k @ k.T
...
# computing the the matrix JQ from: 
# A correct formulation for the Orientation Dynamic
# Movement Primitives for robot control in the Cartesian space
def JQ(Q):
    ...
    if np.abs(theta)>0.000000001:
        J[0:3,0] = ( (-stheta + theta * ctheta) / (stheta*stheta) ) * k
        J[0:3,1:4] = theta * I3 / stheta
